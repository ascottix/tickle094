...
void KeyPressDispatcher::expand()
{
    if( count_ == capacity_ ) {
        capacity_ += 16;
        KeyPressDispatcherItem * items = new KeyPressDispatcherItem[ capacity_ ];
...
void KeyPressDispatcher::add( unsigned key, unsigned id, unsigned data )
{
    if( key != 0 ) {
        expand();
...
TJoystick * KeyPressDispatcher::addJoystick( unsigned id, unsigned kleft, unsigned kright, unsigned kup, unsigned kdown )
{
    int index = -1;
...
void KeyPressDispatcher::clear()
{
    count_ = 0;
}

bool KeyPressDispatcher::handle( unsigned key, unsigned param, TMachine * machine )
{
    bool result = false;
...
void KeyPressDispatcher::notifyJoysticks( TMachine * machine )
{
    for( int i=0; i<joysticks_.count(); i++ ) {
        TJoystick * joystick = (TJoystick *) joysticks_.item(i);
