...
public:
    ...
    */
    ...
    /**
        ...
        @return true if the CPU accepted the interrupt, false otherwise
                (i.e. interrupts are disabled)
    */
    bool interrupt( unsigned char data );
    ...
    /** 
        ...
        Note that if a CPU is left running for enough time without calling
        setCycles(), this counter will overflow!
    */
...
protected:
    ...
    /** Computes parity, sign and zero flags from accumulator, sets halfcarry, resets others. */
    void setFlagsSZ010P00() {
        F = Halfcarry | PSZ_[A];
    }

    /** Computes parity, sign and zero flags from accumulator, preserves others. */
    void setFlagsSZ0x0Pxx() {
        F = (F & (Carry | Halfcarry | Subtraction)) | PSZ_[A];
    }

    /** Computes parity, sign and zero flags from accumulator, resets all others. */
    void setFlagsSZ000P00() {
        F = PSZ_[A];
    }
...
private:
    ...
    unsigned    cycles_;    // Number of CPU cycles spent in the current instruction
    unsigned    t_cycles_;  // Number of CPU cycles elapsed since last reset (or call to setCycles)
