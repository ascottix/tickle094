...
bool SpaceInvaders::initialize( TMachineDriverInfo * info )
{
    ...
    resourceHandler()->add( EfUfo,         "0.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfShot,        "1.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfBomb,        "2.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfTargetHit,   "3.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfStep1,       "4.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfStep2,       "5.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfStep3,       "6.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfStep4,       "7.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfUfoHit,      "8.wav", 0x0000, efSample, 0 );
    resourceHandler()->add( EfExtendPlay,  "9.wav", 0x0000, efSample, 0 );
...
SpaceInvaders::SpaceInvaders( SpaceInvadersBoard * board ) :
    ufo_hit_( 82, 82 ),     // Data obtained by simulation in a "Spice" program
    target_hit_( 2, 90 ),
    shot_( 8400 * 2 ),
    flash_( 8400 / 2 ),
    flash_rc1_( flash_, 560, Micro(0.1) ),
    flash_rc2_( flash_rc1_, Kilo(6.8), Micro(0.1) ),
    walk_timer_( 0, Kilo(75), Micro(0.1) ),   // RA=variable, RB=75 Kohm, C=0.1 uF
    walk_rcfilter_1_( 100, Micro(4.7) ),   // R=100 ohm, C=4.7 uF
    walk_rcfilter_2_( 100, Micro(10) ),    // R=100 ohm, C=10 uF
    extend_play_timer_( Kilo(100), Kilo(47), Micro(1) ) // RA=100 Kohm, RB=47 Kohm, C=1 uF
{
    int i;
...
void SpaceInvaders::run( TFrame * frame, unsigned samplesPerFrame, unsigned samplingRate )
{
    ...
    // Set a minimum number of voices
    frame->getMixer()->getBuffer( chMono, 0, 0 )->setVoices( TMath::max( 4, frame->getMixer()->maxVoicesPerChannel() ) );
...
void SpaceInvaders::renderAnalogSounds( TFrame * frame, unsigned samplesPerFrame, unsigned samplingRate )
{
    ...
    if( main_board_->port5o_ & 0x0F ) {
        if( ! walk_timer_.active() ) {
            ...
            walk_timer_.setVariableResistor( (1/wr)*1000*1.4 ); // 1.4 is hand-tuned... have to discover why
    ...
    if(main_board_->port3o_ & 0x10 ) {
        ...
        // Add the vibrato (some would say, more precisely, tremolo)
