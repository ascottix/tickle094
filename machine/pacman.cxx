...
static int setPacmanHiScore( unsigned char * ram, int score )
{
    int x = 0;
    int i;
    
    for( i=0; i<8; i++ ) {
        ram[ 0x43ED + i ] = 0x40; // Blank
    }
    
    i = 0;
    while( score > 0 ) {
        x |= (score % 10) << (i*4);
        ram[ 0x43ED + i++ ] = score % 10;
        score /= 10;
    }
...
static int getPacmanHiScore( const unsigned char * ram )
{
    int score = 0;
    
    for( int i=3; i>=0; i-- ) {
        unsigned b = ram[ 0x4E88+i ];
        score = 100 * score + 10 * (b >> 4) + (b & 0x0F);
    }
